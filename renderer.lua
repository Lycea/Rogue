...
function renderer.get_name_under_mouse()
    ...
    x,y = camera:CoordWithoutCam(gvar.mouse_coords[1],gvar.mouse_coords[2])
    local mouse_x ,mouse_y = floor(x/gvar.constants.tile_size),floor(y/gvar.constants.tile_size)
...
function renderer.render_bar(name,x,y,width,value,max,bar_col,back_col,string_col,show_num)
  ...
  love.graphics.rectangle("fill",bar_start,y,width,gvar.constants.ui_tile_size)
  love.graphics.setColor(bar_col)
  love.graphics.rectangle("fill",bar_start,y,bar_width,gvar.constants.ui_tile_size)
  love.graphics.setColor(gvar.constants.colors.default)
  love.graphics.rectangle("line",bar_start,y,width,gvar.constants.ui_tile_size)
...
function renderer.draw_turn_infos(dungeon_level)
  ...
  --love.graphics.rectangle("fill",(gvar.constants.map_width+1)*10,(gvar.constants.map_height+1)*10,gvar.constants.scr_width,gvar.constants.scr_height)
    ...
    renderer.render_bar("hp: ",gvar.constants.ui_tile_size*3,(gvar.constants.map_height+3)*gvar.constants.ui_tile_size,
        10*gvar.constants.ui_tile_size,gvar.player.fighter.hp,gvar.player.fighter:get_hp(),
        gvar.constants.colors.light_red,gvar.constants.colors.dark_red)
    
    
    --exp bar (?)
    renderer.render_bar("exp: ",
        gvar.constants.ui_tile_size*3,
        (gvar.constants.map_height+5)*gvar.constants.ui_tile_size,
        10*gvar.constants.ui_tile_size,gvar.player.level.current_xp,gvar.player.level:expToNextLevel(),
        gvar.constants.colors.light_blue,gvar.constants.colors.blue)
    
    gvar.message_log:draw()
    
    love.graphics.print("Depth: "..dungeon_level,gvar.constants.ui_tile_size*3,(gvar.constants.map_height+7)*gvar.constants.ui_tile_size)
    --menue("This is a test header,it tests heading",{"blah","test","noch was","meh"},0,0,scr_width)
end


function renderer.draw_inventory()
  menues.invi_menue("Press key next to item to use or ESC to exit",gvar.player.inventory,gvar.constants.ui_tile_size*1,gvar.constants.ui_tile_size*1,gvar.constants.ui_tile_size*30,gvar.constants.scr_height -gvar.constants.ui_tile_size*2)
  
  menues.menue("Stats",{"Defense: "..gvar.player.fighter:get_def(),"Attack: "..gvar.player.fighter:get_power()},gvar.constants.ui_tile_size*35,gvar.constants.ui_tile_size*1,gvar.constants.ui_tile_size*20,gvar.constants.ui_tile_size*10)
  ...
  menues.menue("Equipment",{slot_main.."\n"..slot_off} ,gvar.constants.ui_tile_size*35,gvar.constants.ui_tile_size*12,gvar.constants.ui_tile_size*20,gvar.constants.ui_tile_size*39)
...
function renderer.render_all(entities,game_map,screen_width,screen_height)--could be adjusted to work without params,but lets see
    ...
    if gvar.game_state == glib.GameStates.LEVEL_UP then
        local x_off = gvar.constants.scr_width/2 -gvar.constants.ui_tile_size*10
        local y_off = gvar.constants.scr_height/2 -gvar.constants.ui_tile_size*6.5
        menues.level_up_menue("Select a state to increase:",x_off,y_off,gvar.constants.ui_tile_size*20,gvar.constants.ui_tile_size*13)
    end
    ...
    if gvar.game_state == glib.GameStates.TARGETING then
      ...
      love.graphics.circle("line",gvar.targeting_tile.x*gvar.constants.ui_tile_size +gvar.constants.ui_tile_size/2,
          gvar.targeting_tile.y*gvar.constants.ui_tile_size +gvar.constants.ui_tile_size/2,
          (gvar.target_range or 1)*(gvar.constants.ui_tile_size/2+1))
      love.graphics.setColor(255,0,0,75)
      love.graphics.circle("fill",gvar.targeting_tile.x*gvar.constants.ui_tile_size +gvar.constants.ui_tile_size/2,
          gvar.targeting_tile.y*gvar.constants.ui_tile_size +gvar.constants.ui_tile_size/2,
          (gvar.target_range or 1)*(gvar.constants.ui_tile_size/2+1))
    ...
    if gvar.player.last_target ~= 0 then
        renderer.render_bar("enemy's hp:",
            (gvar.constants.scr_width)-20*gvar.constants.ui_tile_size,
            (gvar.constants.map_height+3)*gvar.constants.ui_tile_size,
            10*gvar.constants.ui_tile_size,
