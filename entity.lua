...
function Entity.get_blocking_entitis_at_location(destin_x,destin_y)
...
local paths =require(BASE.."components.paths")


local base_pathmaker = paths()
paths:setBlocked(
    function(x,y)
      if gvar.map:is_blocked(x,y) == false and Entity.get_blocking_entitis_at_location(x,y)==nil then
        return false
      else
        return true
      end
...
function Entity:save()
    ...
    dl.offset_push()
    local pos =dl.add_offset()..'"x":'..self.x..",\n"..dl.add_offset()..'"y":'..self.y..",\n"
    local color = dl.add_offset()..'"color":"'..self.color..'",\n'
    
    local name = dl.add_offset()..'"name":"'..self.name..'",\n'
    local blocks = self.blocks==true and '"blocks": true,\n' or '"blocks": false,\n'
          blocks = dl.add_offset()..blocks
    ...
    if self.fighter ~= nil then
        fighter =dl.add_offset()..'"fighter":{\n'..self.fighter:save()..dl.add_offset().."}\n"
    ...
    if self.item ~=nil then
        ...
        item =dl.add_offset()..'"item":{\n'..item_txt..dl.add_offset().."}\n"
    ...
    if self.inventory~= nil then
        inventory=dl.add_offset()..'"inventory":{\n'..self.inventory:save()..dl.add_offset().."}\n"
        table.insert(tmp_string_list,inventory)
    end 
    
    local stairs_ = ""
    if self.stairs_ ~= nil then
        stairs_ = dl.add_offset()..'"stairs":{\n'..self.stairs_:save()..dl.add_offset().."}\n"
    ...
    if self.level ~= nil then
        level=dl.add_offset()..'"level":{\n'..self.level:save()..dl.add_offset().."}\n"
    ...
    if self.equippment ~= nil then
        ...
        equippment_=dl.add_offset()..'"equippment":{\n'..self.equippment:save()..dl.add_offset().."}\n"
        table.insert(tmp_string_list,equippment_)
    end
    
    local equippable_ =""
    if self.equippable ~= nil then
        equippable_=dl.add_offset()..'"equippable":{\n'..self.equippable:save()..dl.add_offset().."}\n"
        table.insert(tmp_string_list,equippable_)
    end
    
    local render_order = dl.add_offset()..'"render_order":'..self.render_order.."\n"
...
function Entity:move_towards4(target_x,target_y)
  ...
  if  gvar.map:is_blocked(self.x+dx,self.y+dy)==false and
      Entity.get_blocking_entitis_at_location(self.x+dx,self.y+dy)==nil then
      self:move(dx,dy)
  end
end
...
function Entity:draw()
  if gvar.fov_map[self.y][self.x]==true then
    
    love.graphics.setColor( gvar.constants.colors[self.color] or {255,0,0})
    love.graphics.rectangle("fill",self.x*gvar.constants.tile_size,
        self.y*gvar.constants.tile_size,gvar.constants.tile_size,gvar.constants.tile_size)
